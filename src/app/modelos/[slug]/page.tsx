...
export async function generateMetadata({ params }: ModelPageProps): Promise<Metadata> {
  ...
  const title = `${model.name} | Modelo Profesional HostPro Panamá`;
  const description = `${model.role} en Panamá. ${model.languages}${model.physical.height ? ` · ${model.physical.height}` : ""}. Disponible para eventos corporativos, activaciones BTL y producciones audiovisuales.`;
      ...
      <main className="min-h-screen bg-[#0a0a0a] text-white py-28">
        <div className="max-w-7xl mx-auto px-6 md:px-12">
          <div className="mb-8 md:mb-10 space-y-4">
            <nav aria-label="Breadcrumb" className="text-[11px] uppercase tracking-[0.14em] text-white/45 flex items-center gap-2">
              <Link href="/" className="hover:text-white transition-colors">Inicio</Link>
              <span>/</span>
              <Link href={collectionRoute} className="hover:text-white transition-colors">Catálogo</Link>
              <span>/</span>
                ...
                {instagramHandle && (
                  <p>
                    ...
                    <Link
                      ...
                      className="hover:text-[#d4b200] transition-colors"
                    >
                      @{instagramHandle}
                    </Link>
                  </p>
